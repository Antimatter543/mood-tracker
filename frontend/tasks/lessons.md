...
- Under `markingType="custom"`, ONE `markedDates[day]` entry composes TWO layers on the day cell: `customStyles.container.backgroundColor` + `customStyles.text.color` (the mood-colored cell + day number) AND `marked:true` + `dotColor` (a small dot under the number). Verified from the installed source (`react-native-calendars@1.1314`: `calendar/day/index.js` → `BasicDay` `renderContent()` always renders `<Marking type='custom' marked dotColor/>` → `<Dot>`, dot `opacity:0` until `marked`). So the activity-dot layer merges onto the mood layer on the SAME object — no multi-dot type, no second pass.
- **Date doctrine holds**: SQL only range-filters raw UTC instants (`monthWindowBounds`, works for ANY visible month — the old `loadMonthData` was hardcoded to `new Date()`); all day-keying is JS (`dailyAverageRows` / `activityDaySet` / `localDateString`). Reused `WEEKLY_MOOD_AVERAGES` / `ENTRIES_FOR_ACTIVITY_IN_RANGE` / `ACTIVITY_CORRELATION` — no new SQL, so the `queriesNoDateBucketing` invariant is untouched.
- **Day-number readability** (`buildCalendarMarkers`, theme-parameterized — NO hardcoded mood palette, only the canonical `moodColor` scale): composite the marker's rgba over the card bg and pick the higher-WCAG-contrast of a light/dark text pair (always ≥ ~4.1:1, both dark and light themes). A YIQ≥128 heuristic picks sub-optimal text near mid-tones and was replaced.
- The day-press summary uses the in-tree `OverlayModal` (never a native `<Modal>` — the repo's hard Fabric rule).
- Tests: `__tests__/calendarMarkers.test.ts` (scale + WCAG contrast + composition), `activityDays` / `monthWindow` / `daySummary` unit tests (Brisbane UTC+10 edges), `activityDaysQuery.integration.test.ts` (node:sqlite: JOIN + `activity_id` filter + inclusive BETWEEN boundary + local keying).

**Rung**: reference (+ `queriesNoDateBucketing` invariant test guards the SQL side; the marker/keying transforms are all pure + unit-tested).

**Date**: 2026-07-25

## 2026-07-13: Statistics tab went fully BLANK ~3s after showing content — a reanimated `transform` animation on the `flex:1` `Layout` container corrupts layout on Fabric when children re-lay-out

**Mistake**: `Layout` (`components/PageContainer.tsx`) wrapped the full-height content branch (`useScrollView={false}`) in `<Animated.View style={[fullHeightContent /* flex:1 */, animatedContentStyle]}>`, where `animatedContentStyle` was a live `useAnimatedStyle` returning `opacity` + `transform:[{translateY}]` (a fade+slide entrance). On the Statistics screen this blanked the whole tab: content painted for ~1s, then vanished. The a11y node tree proved the content was NOT unmounted — every chart View still existed but was positioned ~1.6k px **below** the viewport (inverted `boundsInScreen` rects), i.e. the whole subtree was shoved off-screen. There was **no JS re-render and no error** at the moment it blanked — a pure native/UI-thread layout corruption. Root cause: on Fabric + reanimated 4.3.1, a live animated `transform` on a **`flex: 1`** container gets applied against a stale measured frame when that container's children re-lay-out after mount. Statistics has ~8 charts that each resolve async data and re-render over ~3s, so it reproduced reliably; the lighter Timeline (same `useScrollView={false}` branch, same Animated.View) and Home/Insights (the `useScrollView={true}` branch, where the Animated.View is content-sized, NOT flex:1) never re-lay-out that way, so they were fine — which is exactly why the bug looked "isolated to Stats" and sent the first guess (ActivityExplorer, the new code on that screen) down the wrong path.

**Rule**: (a) **Never attach a live reanimated `useAnimatedStyle` to a `flex:1` full-height container whose children re-lay-out after mount.** The full-height branch of `Layout` now renders a plain static `View`; the fade+slide entrance is kept only for the content-sized `Animated.View` inside the ScrollView branch (Home/Insights), where it's safe. Verified on-device that the animated PROPERTY is irrelevant — an opacity-only animatedStyle blanked it too; only removing the animatedStyle from the flex:1 view fixed it. So the fix is structural (no animatedStyle on flex:1), not "animate a safer property." (b) Regression guard: `__tests__/layoutFullHeightNoTransform.test.tsx` asserts the `useScrollView={false}` branch renders children with NO `transform` anywhere — it FAILS on the pre-fix code (shows the offending `transform:[{translateY:20}]` on the flex:1 View) and passes on the fix. The native off-screen push itself is not jest-reproducible, but reintroducing the transform-animated container is caught deterministically.

**Meta**: Same debugging discipline as the same-day "Home doesn't update" fix — **on-device console.log + the a11y/uiautomator tree are the source of truth, not source-reading.** The task's prime suspect (the new `ActivityExplorer` on the Stats screen) was wrong: a one-line bisect (comment out `<ActivityExplorer/>`, cold-restart) still blanked, and a null-hypothesis check (Timeline/Insights stay rendered after the same 8s settle) proved it was Stats-content-specific. The a11y dump's *inverted* `boundsInScreen` rects (`top > bottom`, content at y≈1928 below a 1852 clip) — not the screenshot — are what revealed "laid out off-screen" vs "unmounted/frozen", which flipped the whole diagnosis from a React lifecycle bug to a native layout bug. Also: cold-restart to repro, never trust a Fast-Refresh remount (it cleared the bad state and made the bug look fixed when it wasn't).

...
const StatSummaryCard: React.FC = () => {
    ...
    // Sync loader that kicks off the async work and returns a cleanup — the
    // `active` flag (flipped on blur/unmount by useDataRefresh) gates every
    // setState so a late query can't update an unmounted card. Mirrors the
    // guard pattern in stats.tsx / ActivityCorrelationChart.
    const fetchSummary = useCallback(() => {
            ...
            (async () => {
                try {
                    ...
                    // 2-axis mood-state from the window's RECORDED days (no
                    // gap-fill, so real swings survive). Pass the same MA slope
                    // the chart uses so the trend direction matches the line.
                    setMoodState(
    ...
    const tiles: {
        ...
        {
            // The mood-state label is free text ("Climbing through ups & downs")
            // and the swing subtitle is dynamic — let both wrap/shrink so neither
            // clips in the 50%-width tile.
            id: 'trend',
            icon: trendIcon,
            value: trendValue,
            label: trendSubtitle,
                ...
                {tiles.map((tile) => (
                    <View key={tile.id} style={styles.tile}>
                        <StatTile
                            ...
                            label={tile.label}
                            color={tile.color}
                            valueLines={tile.valueLines}
                            shrinkToFit={tile.shrinkToFit}
                        />
                    </View>
                ))}
            </View>
